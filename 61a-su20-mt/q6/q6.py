...
def copycat(lst1, lst2):
    ...
    ['c', 'c', 'c']
    """
    def copycat_helper(lst, nums, lst_org):
        if len(nums) == 0 or len(lst_org) == 0:
            return lst
        if nums[0] >= 0:
            [lst.append(lst_org[0]) for _ in range(nums[0]) ]
        else:
            lst = lst[:len(lst)+nums[0]]
        return copycat_helper(lst,nums[1:],lst_org[1:])

    return copycat_helper([],lst2,lst1)

...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        cards_tmp = opponent.deck.cards.copy()
        for item in opponent.deck.cards:
            if item.attack == other_card.attack and item.defense == other_card.defense:
                cards_tmp.remove(item)
        opponent.deck.cards = cards_tmp

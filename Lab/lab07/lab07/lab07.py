...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            return [[]]
        elif s[0] < prev:#即之前记录的数字大于s[0]，无法保持递增形式
...
def make_glookup(class_assignments):
    ...
    def glookup(key,value):
        nonlocal user_score,total_score
        user_score += value
        total_score += class_assignments[key]

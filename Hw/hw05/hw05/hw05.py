def make_bank(balance):
    ...
    def bank(message, amount):
        ...
        if message == 'withdraw':
            ...
            #return balance - amount if balance > amount else "Insufficient funds"
        #"*** YOUR CODE HERE ***"
...
def make_withdraw(balance, password):
    ...
    err_times = 0
    "*** YOUR CODE HERE ***"
    def make_withdraw_helper(amount,type_password):
        nonlocal incorrect_pass,balance
        nonlocal err_times
        if err_times > 2:
            return "Too many incorrect attempts. Attempts: {}".format(incorrect_pass)
        if type_password != password:
            incorrect_pass.append(type_password)
            err_times += 1
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    def make_joint_helper(deposit,str):
        if str == new_pass or str == old_pass:
            return withdraw(deposit,old_pass)
        else:
           return withdraw(deposit,str)

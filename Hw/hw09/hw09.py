...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(index,odd_status,even_status,res):
        if index > n:
            return res
        if odd_status == 0:
            return helper(index+1,1,0,res + even_term(index))
        else :
            return helper(index+1,0,1,res + odd_term(index))

    return helper(1,1,0,0)
...
def mutate_reverse(link):
    ...
    def helper(sub_link):
        ...
        if sub_link.rest is Link.empty:
            lists.append(sub_link.first)
            return
        helper(sub_link.rest)
        lists.append(sub_link.first)

    def helper_2(lists,sub_link):

        if len(lists) == 1:
            sub_link.first = lists[0]

        else:
            sub_link.first = lists[0]
            helper_2(lists[1:],sub_link.rest)

    ...
    def vend(self):
        if self.product_num == 0:
            ...
            if self.fund < self.price:
                ...
                self.product_num -= 1
                more_fund = self.fund - self.price
                self.fund = 0
                if more_fund == 0:
                    return 'Here is your {}.'.format(self.product)
                else:
                    return 'Here is your {} and ${} change.'.format(self.product,more_fund)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        more = Mint.current_year - self.year
        return self.cents + (0 if more <= 50 else more - 50)
class Nickel(Coin):
    cents = 5
...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def bst_min(tree,minum):
        if tree.label < minum:
            minum = tree.label
        if tree.is_leaf():
            return minum
        else :
            minum = min(bst_min(b,minum) for b in tree.branches)
            return minum

    def bst_max(tree,maxm):
        if tree.label > maxm:
            maxm = tree.label
        if tree.is_leaf():
            return maxm
        else:
            maxm = max(bst_max(b,maxm) for b in tree.branches)
            return maxm
    ...
    if len(t.branches) > 2:
       return False
        ...
        if bst_max(t.branches[0],t.branches[0].label) > t.label or bst_min(t.branches[1],t.branches[1].label) <= t.label:
            return False
...
def remove_all(link , value):
    ...
    "*** YOUR CODE HERE ***"
    st = link
    while st is not Link.empty and st.rest is not Link.empty:
        while st.rest is not Link.empty and st.rest.first == value:
            st.rest = st.rest.rest
        st = st.rest

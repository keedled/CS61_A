...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #new_expr = map(scheme_eval(expr,env))
        #new_expr =
        #return scheme_apply(scheme_eval(first,env),new_expr)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        new_frame.bindings = self.bindings.copy()
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        #new_frame.define(self.formals,args.first)
        return new_frame
        "*** YOUR CODE HERE ***"
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        first,rest = expressions.first,expressions.rest.first
        env.define(first,scheme_eval(rest,env))
        return first
        "*** YOUR CODE HERE ***"
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    while curr.rest is not nil:
        if is_false_primitive(scheme_eval(curr.first,env)):
            return False
        curr = curr.rest
    if curr is not nil:

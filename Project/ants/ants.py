...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        Asks the insect to remove itself from the current place. This method exists so
            it can be enhanced in subclasses.
        """
        if isinstance(self.ant,QueenAnt) == False:
            if isinstance(self.ant,ContainerAnt) and self.ant.contained_ant and isinstance(self.ant.contained_ant,QueenAnt) and self.ant.contained_ant.True_Queen:
                return
            else:
                insect.remove_from(self)
        else :
            if self.ant.True_Queen == False:
                insect.remove_from(self)
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if self.can_contain(place.ant):#如果这个昆虫（self）能把原来这个位置上的昆虫contain，那更新这个位置上的昆虫。
                self.contain_ant(place.ant)
                place.ant = self
            elif place.ant.can_contain(self):#如果这个位置本来的昆虫就是一个contain类型的，那就可以直接包含了self。
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        loc_place = self.place
        while loc_place.entrance is not None:
            if len(loc_place.bees) and dis <= self.max_range and dis >= self.min_range:
                return rANTdom_else_none(loc_place.bees)
            else:
                dis += 1
                loc_place = loc_place.entrance
        if len(loc_place.bees) and isinstance(loc_place,Hive) == False and dis <= self.max_range and dis >= self.min_range:
            return rANTdom_else_none(loc_place.bees)
        return None
...
class ShortThrower(ThrowerAnt):
    ...
    #     loc_place = self.place
    #     while loc_place.entrance is not None:
    #         if len(loc_place.bees) and dis <= self.max_range:
    #             return rANTdom_else_none(loc_place.bees)
    #         else:
    #             loc_place = loc_place.entrance
    #             dis += 1
    #     if len(loc_place.bees) and isinstance(loc_place,Hive) == False and dis <= self.max_range:
    #         return rANTdom_else_none(loc_place.bees)
    #     return None
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
...
class LongThrower(ThrowerAnt):
    ...
    #     loc_place = self.place
    #     while loc_place.entrance is not None:
    #         if len(loc_place.bees) and dis >= self.min_range:
    #             return rANTdom_else_none(loc_place.bees)
    #         else:
    #             dis += 1
    #             loc_place = loc_place.entrance
    #     if len(loc_place.bees) and isinstance(loc_place,Hive) == False and dis >= self.min_range:
    #         return rANTdom_else_none(loc_place.bees)
    #     return None
    # BEGIN Problem 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        unlucky_bee = rANTdom_else_none(self.place.bees)
        if self.digesting <= 0 and unlucky_bee:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.True_Queen:
            ...
            while curr_place is not None and curr_place.exit is not None:
                if curr_place.ant:
                    if curr_place.ant.is_buffed == False:
                    ...
                    if isinstance(curr_place.ant,ContainerAnt) and curr_place.ant.contained_ant:
                        if curr_place.ant.contained_ant.is_buffed == False:
                            curr_place.ant.contained_ant.is_buffed = True
                            curr_place.ant.contained_ant.damage *= 2
                curr_place = curr_place.exit
            if curr_place and curr_place.ant:
                if curr_place.ant.is_buffed == False:
                    curr_place.ant.is_buffed = True
                    curr_place.ant.damage *= 2
                if isinstance(curr_place.ant, ContainerAnt) and curr_place.ant.contained_ant:

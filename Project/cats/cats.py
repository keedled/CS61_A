...
def about(topic):
    ...
    def Select(paragraph):
        paragraph = set(split(remove_punctuation(lower(paragraph))))
        for word in topic:
...
def accuracy(typed, reference):
    ...
    return T_num * 100 /len(typed_words)
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    diff_list = [diff_function(user_word,x,limit) for x in valid_words]
    words,diff_num = min(zip(valid_words,diff_list),key = lambda item:item[1])
    if diff_num > limit:
        return user_word
    else :
        return words
...
def shifty_shifts(start, goal, limit):
    ...
    def solve(str1,str2,depth):
        ...
        if str1 == '':
            ...
            if str1[0] == str2[0]:
                return solve(str1[1:],str2[1:],depth + 1)
            else :
                return 1 + solve(str1[1:],str2[1:],depth + 1)
    ans = solve(goal[:length],start[:length],1)
    ans += max(len(start),len(goal)) - min(len(goal),len(start))
    if ans > limit:
         return limit + 1
    return ans
    # if len(start) == 0:
...
def meowstake_matches(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:
        return max(len(start),len(goal)) - min(len(goal),len(start))
...
def fastest_words(game):
    ...
    word_min_player = []
    for i in words:
        min_time,index_player = 999,0
        for j in players:
            if time(game,j,i) < min_time:
                min_time = time(game,j,i)
                index_player =  j
        word_min_player.append(index_player)

    for i in words:
        ans[word_min_player[i]].append(all_words(game)[i])
